...
@section('main')
		...
		<div class="alert alert-info col-md-4 col-md-offset-1">
		    ...
		    <strong>揽件统计:</strong>{{ $name }} 总共收揽 <strong>{{ count($things) }}</strong> 件。
		</div>
		<div class="alert alert-success col-md-4 col-md-offset-1">
		    <a href="#" class="close" data-dismiss="alert">
		        &times;
		    </a>
		    <strong>送件统计:</strong>{{ $name }} 总共送达 <strong>{{ $count }}</strong> 件。
		</div>
		    ...
		    <strong>发件统计:</strong>{{ $name }} 还有 <strong>{{ count($things) - $count }}</strong> 件正在运输中。
		</div>
	</div>
@endsection